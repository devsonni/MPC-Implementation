...
%compute the constrains
 for k=1:N+1
     ...
     g = [g, -sqrt((X(1,k)-x_o_1)^2+(X(2,k)-P(12))^2) + (UAV_r+obs_r)]; %limit of obstacle-1
     g = [g, -sqrt((X(1,k)-x_o_2)^2+(X(2,k)-P(13))^2) + (UAV_r+obs_r)]; %limit of obstacle-2
     g = [g, -sqrt((X(1,k)-x_o_3)^2+(X(2,k)-P(14))^2) + (UAV_r+obs_r)]; %limit of obstacle-3
     g = [g, -sqrt((X(1,k)-x_o_4)^2+(X(2,k)-P(15))^2) + (UAV_r+obs_r)]; %limit of obstacle-4
     g = [g, -sqrt((X(1,k)-x_o_5)^2+(X(2,k)-P(16))^2) + (UAV_r+obs_r)]; %limit of obstacle-5
     g = [g, -sqrt((X(1,k)-x_o_6)^2+(X(2,k)-P(17))^2) + (UAV_r+obs_r)]; %limit of obstacle-6
...
t0 = 0;
x0 = [-501;150;80;0;0;0;0;0]; %initial location of UAV and gimbal
xs = [-500;150;0]; %reference for the UAV which is target
...
a_p = zeros(loop_run);
b_p = zeros(loop_run);
y_0_1 = zeros(loop_run);
y_0_2 = zeros(loop_run);
y_0_3 = zeros(loop_run);
y_0_4 = zeros(loop_run);
y_0_5 = zeros(loop_run);
y_0_6 = zeros(loop_run);

main_loop = tic;
while (mpciter < loop_run)
    args.p = [x0;xs;y_o_1;y_o_2;y_o_3;y_o_4;y_o_5;y_o_6];
    
    if (mpciter>100 && mpciter<400)
        y_o_2 = y_o_2 - 1;
    end
    if (mpciter>200 && mpciter<500)
        y_o_3 = y_o_3 + 1;
    end
    if (mpciter>300 && mpciter<600)
        y_o_4 = y_o_4 - 1;
    end
    if (mpciter>500 && mpciter<800)
        y_o_5 = y_o_5 + 1;
    end
    if (mpciter>600 && mpciter<900)
        y_o_6 = y_o_6 - 1;
    end
    if (mpciter>1000 && mpciter<1300)
        y_o_1 = y_o_1 + 1;
    end
    
    y_0_1(mpciter+1) = y_o_1;
    y_0_2(mpciter+1) = y_o_2;
    y_0_3(mpciter+1) = y_o_3;
    y_0_4(mpciter+1) = y_o_4;
    y_0_5(mpciter+1) = y_o_5;
    y_0_6(mpciter+1) = y_o_6;
    ...
    ff_value = ff(u',args.p(1:11)); % compute OPTIMAL solution TRAJECTORY
...
for j=1:loop_run
    ...
    hold on;
    surf(X+x_o_1,Y+y_0_1(j),Z);
    hold on;
    surf(X+x_o_2,Y+y_0_2(j),Z);
    hold on;
    surf(X+x_o_3,Y+y_0_3(j),Z);
    hold on;
    surf(X+x_o_4,Y+y_0_4(j),Z);
    hold on;
    surf(X+x_o_5,Y+y_0_5(j),Z);
    hold on;
    surf(X+x_o_6,Y+y_0_6(j),Z);
    hold on;
    surf(X+x_o_7,Y+y_o_7,Z);
    hold on;
    surf(X+x_o_8,Y+y_o_8,Z);
...
myWriter = VideoWriter('Dynamic Obstacle 3', 'MPEG-4');
